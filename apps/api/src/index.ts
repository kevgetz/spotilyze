...
app.get('/stats/top-artists', verifyToken, async (c) => {
  try {
    ...
    const topArtists = await db.collection('streaming_records').aggregate([
      { $match: { userId: userId } },
      {
        $group: {
...
app.get('/stats/top-albums', verifyToken, async (c) => {
  try {
    ...
    const topAlbums = await db.collection('streaming_records').aggregate([
      { $match: { userId: userId } },
      {
        $match: {
...
app.get('/stats/top-songs', verifyToken, async (c) => {
  try {
    ...
    const topSongs = await db.collection('streaming_records').aggregate([
      { $match: { userId: userId } },
      {
        $match: {
...
app.get('/stats/summary', verifyToken, async (c) => {
  try {
    ...
    const summary = await db.collection('streaming_records').aggregate([
      { $match: { userId: userId } },
      {
        $group: {
          _id: null,
      ...
      },
      {
        $project: {
          ...
          timeSpanYears: { 
            $round: [{ 
              $divide: [
                { $subtract: [{ $dateFromString: { dateString: "$latestDate" }}, { $dateFromString: { dateString: "$earliestDate" }}] }, 
                31557600000
              ] 
            }, 1] 
          },
...
app.get('/stats/monthly-activity', verifyToken, async (c) => {
  try {
    ...
    const monthlyData = await db.collection('streaming_records').aggregate([
      { $match: { userId: userId } },
      {
        $project: {
...
app.get('/stats/daily-patterns', verifyToken, async (c) => {
  try {
    ...
    const dailyData = await db.collection('streaming_records').aggregate([
      { $match: { userId: userId } },
      {
        $project: {
...
app.get('/stats/artist-distribution', verifyToken, async (c) => {
  try {
    ...
    const artistData = await db.collection('streaming_records').aggregate([
      { $match: { userId: userId } },
      {
        $group: {
          _id: "$master_metadata_album_artist_name",
